function HomeStoryModule(data, startRatio, useCollapse) {
  ...
  function updateStories() {
    for (var i = 0; _numOfStories > i; i++) {
      ...
      story.setExpandedWidth(_storyExpandedW),
        ...
        useCollapse ? story.setRatio(_ratio, !0) : story.setRatioNoOffset(1, !0),
        TweenMax.set(story, {  x: _storyExpandedW * i })
    }
  }

  //toma el valor y del scroll y lo asigna al offset para moverlo horizontalmente con css 
  function onScroll() {
  ...
  function addStories() {
    ...
    for (var i = 0; _numOfStories > i; i++) {
      ...
      if (story.onStoryClick = onStoryClick,
        modelId == i && story.setBodyModelController(), 
        _stories.push(story), 
