function MenuContactInfo(data, guides) {
    function startLayoutListen() {
        getColumnsTextWidth(), guides.addEventListener(GuideLines.ON_UPDATE, updateLayout), updateLayout()
    }

    function addHeadline() {
        _headline = Text.getNewReg(13), _headline.style.color = UIColors.WHITE, _headline.innerHTML = ContentManager.getChildByAttr(data, "name", "headline").innerHTML, _headline.style.whiteSpace = "nowrap", _instance.appendChild(_headline);
    ...
    function addColumns() {
        for (var columns = ContentManager.getChildByAttr(data, "name", "columns"), l = columns.children.length, i = 0; l > i; i++) {
            ...
            column.lineHeightOffset = 1, column.style.color = UIColors.FONT_MED_ON_WHITE, column.style.whiteSpace = "nowrap", column.innerHTML = columns.children[i].innerHTML, column.updateLineHeight(), TweenMax.set(column, {
    ...
    var _headline, OFFSET_TOP = 19,
        COLUMN_SPACING = 20,
        _isMobileMode = !1,
        _columns = [];
    _instance.init = function () {
        addHeadline(), addColumns(), Text.listenForSize(_columns, startLayoutListen)
