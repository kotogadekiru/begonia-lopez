function TemplateHome(data) {
	...
	var _homeStoryModule, _basicModule, _returnModule;
	...
	function getCurrentIndex() {
		...
		var storyX = _homeStoryModule.aniGetX();
		var numOfStories = _homeStoryModule.getNumOfStories();
	...
	function easeToStory(storyIndex, speed) {
		speed = speed != null ? speed : 0.4;//0.4
		var storyWidth = _homeStoryModule.getExpandedStoryWidth();
	...
	function setupAndAddModules() {
		...
		_homeStoryModule = new HomeStoryModule(storyData, homeStartOffset, _useCollapse);
		_homeStoryModule.onStoryClick = onStoryClick;
		...
		_instance.addModule(_homeStoryModule);
	...
	function onOverScrollingTop() {
		//console.log("calling overScrollingTop on TemplateHome after event dispatch?");
		/*
		_offscrollTicks++;

		if (!_isPhychoOpen && _offscrollTicks > 30) {
			TweenMax.killTweensOf(_offsetEase);
			TweenMax.to(_offsetEase, 0.6, {
	...
	function onStoryClick(storyNumber) {
		...
		//ok esto funciona!!
		_homeStoryModule.expandSubstories(storyNumber);
