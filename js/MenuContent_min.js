function MenuContent() {
    ...
    function addSocial() {
        ...
        _social = new MenuSocial(socialData, _guides), _instance.appendChild(_social), _social.init()
    }

    function addContactInfo() {
        var contactData = ContentManager.getChildByAttr(_menuData, "name", "contact");
        _contact = new MenuContactInfo(contactData, _guides), _instance.appendChild(_contact), _contact.init()
    }
    ...
    function updateLayout() {
        ...
        _width = _guides.getGuide("end"), 400 > _width ? (_width = Assets.RESIZE_MANAGER.getWindowWidth(),
        _contact.setMobileMode()) : _contact.setDesktopMode(),
        _social.style.display = Assets.RESIZE_MANAGER.getWindowHeight() < 350 ? "none" : "inline",
        Assets.RESIZE_MANAGER.getWindowHeight() > 300 ? _contact.style.display = "inline" : Assets.RESIZE_MANAGER.getWindowHeight() < 300 && (_contact.style.display = "none"),
        _isOpen || TweenMax.set(_instance, {
                x: -_width
    ...
    return _instance.init = function () {
        ...
        _instance.openWide = function (speed) {
            null == speed && (speed = .5), TweenMax.to(_instance, speed, {
            ...
            null == speed && (speed = .2), ease = null != ease ? ease : Quart.easeOut, TweenMax.to(_instance, speed, {
                x: -_width,
                width: _width,
                ease: ease
            }), _isOpen = !1
