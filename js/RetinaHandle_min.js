function RetinaHandle() {
    ...
    function calculate() {
        clearTimeout(_refreshTimeout), _pixelRatio = window.devicePixelRatio, _pixelRatio || (_pixelRatio = 1);
        ...
        for (i = 0; l > i; i += 1) {
            ...
            scaleDist = Math.abs(currScale - _pixelRatio), minScaleDist > scaleDist && (minScaleDist = scaleDist, targetScale = currScale);
        }
        _assetsScale = targetScale, _assetsScalePct = 1 / _assetsScale;
        var oldAssetsScaleStr = _assetsScaleStr;
        _assetsScaleStr = "@x" + _assetsScale, _assetsScaleStr != oldAssetsScaleStr ? (traceValues(), 
        _instance.dispatchEvent(RetinaHandleEvents.CHANGE)) : _isFirstRun && (_isFirstRun = !1, traceValues()), 
    ...
    return init(), _instance.setSupportedScales = function (array) {
