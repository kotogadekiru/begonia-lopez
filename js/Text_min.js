Text = {}, Text.listenForSize = function (arr, callback, runNum) {
...
Text.getNewMed = function (fontSize) {
    ...
    div //lo que se devuelve es el div en definitiva
}, Text.getNewReg = function (fontSize) {
    var div = document.createElement("div");
    return div.style.position = "absolute", div.style.fontFamily = Text.FontFamilyRegular, div.lineHeightOffset = 0, div.lineHeightScale = 1, div.updateLineHeight = function () {
        Text.baseLineheightOnFontSizeLight(div)
    }, fontSize && (div.style.fontSize = fontSize + "px", div.updateLineHeight()), CSS.antialiasText(div), div
}, Text.getNewLight = function (fontSize) {
    var div = document.createElement("div");
    return div.style.position = "absolute", div.style.fontFamily = Text.FontFamilyLight, div.lineHeightOffset = 0, div.lineHeightScale = 1, div.updateLineHeight = function () {
        Text.baseLineheightOnFontSizeLight(div)
    }, fontSize && (div.style.fontSize = fontSize + "px", div.updateLineHeight()), CSS.antialiasText(div), div
    ...
    var lineScale = 1;
    div.lineHeightScale && (lineScale = div.lineHeightScale), div.style.lineHeight = offset + Math.round(parseInt(div.style.fontSize) * lineScale) + "px"
};