function PageTemplate() {
    ...
    function onModuleLoaded() {
        clearTimeout(_timeoutAllModulesLoaded), _modulesLoaded += 1, _timeoutAllModulesLoaded = setTimeout(checkForAllModulesLoaded, 400)
    }
    ...
    function setModuleVisibilty() {
        for (var l = _modules.length, module = null, ratio = 0, i = 0; l > i; i += 1) module = _modules[i], ratio = 1 / module.getWidth() * (_scrollController.currentScroll.y - module.aniGetX() + _instance.visibleWidth), module.visibilityRatio(ratio)
    }
    ...
    var _scrollController = null,
        _modules = [];
    _instance.pageHeight = 0, _instance.visibleWidth = 0, _instance.visibleHeight = 0;
    var _modulesLayer = null,
        _modulesLoaded = 0,
        _timeoutAllModulesLoaded = null;
    _instance.init = function (layer) {
        _modulesLayer = document.createElement("div"), _modulesLayer.style.position = "absolute", _instance.appendChild(_modulesLayer);
        var targetLayer = layer ? layer : Assets.LAYER_TEMPLATE;
        targetLayer.appendChild(_instance), _scrollController = Assets.SCROLL_CONTROLLER, Assets.RESIZE_MANAGER.addEventListener(ResizeEvents.RESIZE, _instance.onResize), _instance.setPageHeight(_instance.pageHeight), document.onkeydown = onKeyPress, _scrollController.addEventListener(ScrollController.ON_SCROLL_MOVE, onPageScroll)
        ...
        if ("function" != typeof module.resize_desktop && console.error("module must have resize(width, height) function"), "function" != typeof module.getHeight && console.error("module must have getHeight() function"), "function" != typeof module.kill && console.error("module must have kill() function"), _modulesLayer.appendChild(module), module.setCallbackLoaded(onModuleLoaded), module.setCallbackResize(_instance.resizeModules), module.setCallbackReposition(_instance.repositionModules), module.init(), AnimationUtils.apply(module), insertAfterThisModule) {
            ...
            _modules.splice(index, 0, module), _instance.resizeModules()
    ...
    _instance.templateIn = function () {
        /*
        null != _scrollController && _scrollController.scrollTo(0, 0), 
        Assets.MAIN_MENU.isCollapsed() ? TweenMax.to(Assets.LAYER_BOT, 1, {
            x: 0,
            onComplete: function () {
                ContentManager.nextTemplate()
            },
    ...
    _instance.onResize = function () {
        ...
        if (0 !== l) {
            var xPos = 0;
            if (target) {
                var index = _modules.indexOf(target); - 1 !== index && (module = _modules[index], callResizeOnModule(module, _instance.visibleWidth, _instance.visibleHeight), _instance.repositionModules(module))
            } else {
                for (var i = 0; l > i; i += 1) module = _modules[i], callResizeOnModule(module, _instance.visibleWidth, _instance.visibleHeight), module.aniSetX(xPos), module.aniRender(), xPos += module.getWidth();
                _instance.pageHeight = xPos, _instance.setPageHeight(_instance.pageHeight)
            }
            onPageScroll()
        }
    }, 
    _instance.repositionModules = function (target) {
        ...
        Assets.RESIZE_MANAGER.removeEventListener(ResizeEvents.RESIZE, _instance.onResize), _scrollController.removeEventListener(ScrollController.ON_SCROLL_MOVE, onPageScroll), killModules(), _instance.parentNode.removeChild(_instance)
    };
    var _afterIgnore;
    return _instance.ignoreScrollAfter = function (height) {
        ...
        var totalHeight = _scrollController.getPageHeight(),
            offset = 0,
            height = totalHeight - _scrollController.getScrollRatio() * totalHeight;
        ...
        index >= _modules.length && (index = _modules.length - 1), _scrollController.scrollTo(_modules[index].aniGetX(), 1, Expo.easeInOut)
