function HomeStory(data, storyNumber, bodyTextModel) {
	...
	function addSiteLine() {
		_line = document.createElement("div"), _line.style.position = "absolute", _line.style.backgroundColor = UIColors.LINE_ON_WHITE, _instance.appendChild(_line)
	}

	function addSlideButtons() {
		1 >= _numOfImages || (_leftButton = new RetinaImage("assets/images/logo/arrow_short.png", Assets.RETINA_HANDLE), 
		_rightButton = new RetinaImage("assets/images/logo/arrow_short.png", Assets.RETINA_HANDLE), 
		_leftButton.style.cursor = "pointer", _rightButton.style.cursor = "pointer", 
		Touchable.apply(_leftButton), Touchable.apply(_rightButton), _leftButton.onClick(function () {
	...
	function addImageSlide() {
		for (var slideData = ContentManager.getChildByAttr(data, "name", "images"),
				slides = ContentManager.getChildrenByAttr(slideData, "name", "image"),
				urls = [], l = slides.length, i = 0; l > i; i++) urls.push(slides[i].innerHTML);
		_imageSlider = new ImageSlider(urls), _instance.appendChild(_imageSlider), _imageSlider.init()
	}

	function addNumber() { //se agrega el texto con el numero de la story. como hago para rotarlo?
		_number = Text.getNewMed(95), _number.innerHTML = storyNumber,
			_number.style.color = UIColors.FONT_MED_ON_WHITE,
			//_number.style.transform = "rotate(90deg)",//con esto roto el numero 90grados pero se rompe la transformacion que lo ubica visible
	...
	function addTopBody() {
		...
		txtBase.lineHeightScale = 14 / 13, _bodyTop = new TextArea(bodyData.innerHTML, txtBase), _bodyTop.init(bodyTextModel, TextAreaModel.MODE_LISTEN), _bodyTop.style.color = UIColors.FONT_MED_ON_WHITE, _bodyMask = document.createElement("div"), _bodyMask.style.position = "absolute", _bodyMask.appendChild(_bodyTop), _instance.appendChild(_bodyMask)
	}
	...
	function addBody() {
		...
		txtBase.lineHeightScale = 14 / 13,
			_body = new TextArea(bodyData.innerHTML, txtBase), _body.init(bodyTextModel, _mode),
	...
	//agregar este codigo para leer las substories y 
	function addSubStories() {
		var sub_stories = ContentManager.getChildrenByAttr(data, "name", "sub-stories");//sub-story
		//var sub_story = ContentManager.getChildrenByAttr(sub_stories, "name", "sub-story");//sub-story
		//sub_modelId = getLongestStoryId(sub_stories),
		sub_model = new TextAreaModel;
		sub_model.maxFontSize = 18, 
		BrowserDetect.MOBILE && (sub_model.maxFontSize = sub_model.minFontSize = 13),
		_numOfSubStories = sub_stories.length;
		//console.log("sub-story length",_numOfSubStories);
		

		console.log("sub_story by attribute number");
		for (var i = 0; _numOfSubStories > i; i++) {
		 // var sub_story = new HomeStory(sub_stories[i], _numOfSubStories - i, sub_model);
			console.log("showing subStory", sub_stories[i])
		  /*
		  if (sub_story.onStoryClick = onSubStoryClick,
			sub_modelId == i && sub_story.setBodyModelController(), 
			_sub_stories.push(sub_story), 
			_instance.appendChild(sub_story), 
			story.init(), useCollapse && i == _numOfStories - 1) 
			{
			  for (var reverseStories = [], j = 0; 3 > j; j++) {
	...
	return _buttonContainer.style.position = "absolute",
		...
		_instance.init = function () {
			...
			var slideData = ContentManager.getChildByAttr(data, "name", "images"),
				slides = ContentManager.getChildrenByAttr(slideData, "name", "image");
			_numOfImages = slides.length, addSiteLine(),
