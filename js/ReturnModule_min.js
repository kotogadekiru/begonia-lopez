function ReturnModule(width) {
    function addArrow() {
        _arrow = new RetinaImage("assets/images/logo/arrow-left.png", Assets.RETINA_HANDLE, onArrowLoaded), _arrow.init()
    }

    function onArrowLoaded() {
        _arrow.getContent().style.transform = "rotatey(180deg)", positionArrow(), _instance.appendChild(_arrow)
    }
    ...
    _instance.init = function () {
        addArrow(), Touchable.apply(_instance), _instance.onClick(onClick)
    }, _instance.addLine = function (color) {
        _line = document.createElement("div"), _line.style.position = "absolute", _line.style.backgroundColor = color, _instance.appendChild(_line)
